...
void ourRTSPClient::SetBuffManager(std::shared_ptr<rtsp_client::BufferManager> buff_manager) {
...
static void parseRTSPURL(char const *url, char *&username, char *&password,
  ...
  strncat(urlNoPassword + prefixLength, from, strlen(url) - prefixLength);
}
  ...
  do {
    ...
    if (strcmp(scs.subsession->mediumName(), "video") == 0) {
      ...
      if (strcmp(scs.subsession->codecName(), "PCMA") == 0) {
        pay_load = RTSP_Payload_PCMA;
      } else if (strcmp(scs.subsession->codecName(), "PCMU") == 0) {
        pay_load = RTSP_Payload_PCMU;
      } else {
    ...
    const char* vps = scs.subsession->fmtp_spropvps();
    const char* sps = scs.subsession->fmtp_spropsps();
    const char* pps = scs.subsession->fmtp_sproppps();
    const char* fmtp_sps = scs.subsession->fmtp_spropparametersets();
    ...
    if (pay_load == RTSP_Payload_H264) {
      H264Sink *dummy_sink_ptr = H264Sink::createNew(
          env, *scs.subsession, rtspClient->url(),
          buffer_size, buffer_count, fmtp_sps);
      ...
      dummy_sink_ptr->AddBufManager(our_rtsp_client->GetBuffManager());
      scs.subsession->sink = dummy_sink_ptr;
    } else if (pay_load == RTSP_Payload_H265) {
      H265Sink *dummy_sink_ptr = H265Sink::createNew(
          env, *scs.subsession, rtspClient->url(),
          buffer_size, buffer_count, fmtp_sps, vps, sps, pps);
      ...
      dummy_sink_ptr->AddBufManager(our_rtsp_client->GetBuffManager());
      scs.subsession->sink = dummy_sink_ptr;
    } else if (pay_load == RTSP_Payload_PCMU ||
               pay_load == RTSP_Payload_PCMA) {
...
void shutdownStream(RTSPClient *rtspClient, int exitCode) {
  if (!(reinterpret_cast<ourRTSPClient *>(rtspClient))->get_shut_down()) {
    return;
  }
  (reinterpret_cast<ourRTSPClient *>(rtspClient))->set_shut_down();
  UsageEnvironment &env = rtspClient->envir();  // alias
  StreamClientState &scs =
      (reinterpret_cast<ourRTSPClient *>(rtspClient))->scs;  // alias
